...
install_package_if_missing "nftables"
...
if [[ -f "$INSTALL_DIR/init-nftables-kighmu.sh" ]]; then
  cp "$INSTALL_DIR/init-nftables-kighmu.sh" /usr/local/bin/init-nftables-kighmu.sh
  chmod +x /usr/local/bin/init-nftables-kighmu.sh
  echo "🚀 Initialisation de la table kighmu nftables..."
  /usr/local/bin/init-nftables-kighmu.sh
  echo "✅ Table kighmu nftables initialisée"
fi
