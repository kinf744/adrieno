...
LOCK_FILE="/tmp/auto-clean.lock"
exec 200>"$LOCK_FILE"
flock -n 200 || { echo "[$(date '+%Y-%m-%d %H:%M:%S')] Auto-clean deja en cours — abandon"; exit 0; }
...
REPORT_SECRET=""
# Ordre de priorite : .env > .report_secret > traffic-collect.sh
if [[ -f "$PANEL_ENV" ]]; then
...
if [[ -z "$REPORT_SECRET" ]]; then
    _rs="/opt/kighmu-panel/.report_secret"
    [[ -f "$_rs" ]] && REPORT_SECRET=$(<"$_rs")
fi
if [[ -z "$REPORT_SECRET" ]]; then
...
[[ -z "$REPORT_SECRET" ]] && { echo "[ERR] REPORT_SECRET introuvable"; exit 1; }
...
# ── 0d. UDP Zivpn + Hysteria : mesure trafic via nftables par port ──────────
# Principe :
#   - Créer des compteurs nommés nftables par port (une règle globale par proto)
...
_read_nft_counter() {
    local name="$1"
    nft list counter inet kighmu "$name" 2>/dev/null | grep -oP 'bytes \K\d+' || echo 0
}

# Initialiser les compteurs nftables de comptage UDP (appelé une seule fois)
init_udp_nftables() {
    /usr/local/bin/init-nftables-kighmu.sh
    ...
    if nft list counter inet kighmu udp_zivpn_in &>/dev/null; then
        return
    fi

    nft add counter inet kighmu udp_zivpn_in
    nft add counter inet kighmu udp_zivpn_out
    nft add counter inet kighmu udp_hysteria_in
    nft add counter inet kighmu udp_hysteria_out

    nft add rule inet kighmu input  udp dport "${zivpn_port}"  counter name "udp_zivpn_in"  accept
    nft add rule inet kighmu output udp sport "${zivpn_port}"  counter name "udp_zivpn_out" accept
    nft add rule inet kighmu input  udp dport 20000-50000    counter name "udp_hysteria_in"  accept
    nft add rule inet kighmu output udp sport 20000-50000    counter name "udp_hysteria_out" accept
    echo "[$(TS)] [UDP] Compteurs nftables initialisés" >> "$LOG_FILE"
}

read_udp_upload_bytes() {
    local counter_name="$1"
    _read_nft_counter "$counter_name"
}
...
collect_udp_traffic() {
    ...
    # Initialiser les compteurs nftables si nécessaire
    init_udp_nftables
    ...
    if [[ -f "$zivpn_uf" ]]; then
        ...
        cur_zivpn_up=$(read_udp_upload_bytes "udp_zivpn_in")
        cur_zivpn_dn=$(read_udp_download_bytes "udp_zivpn_out")
    ...
    if [[ -f "$hy_uf" ]]; then
        ...
        cur_hy_up=$(read_udp_upload_bytes "udp_hysteria_in")
        cur_hy_dn=$(read_udp_download_bytes "udp_hysteria_out")
...
# ==========================================
# 5. Nettoyage activity_logs (2 mois)
# ==========================================
clean_activity_logs() {
    [[ $MYSQL_OK -eq 0 ]] && return
    local cutoff; cutoff=$(date '+%Y-%m-%d' -d '2 months ago')
    local count
    count=$(sql "SELECT COUNT(*) FROM activity_logs WHERE created_at < '$cutoff';" 2>/dev/null)
    [[ -z "$count" || "$count" -eq 0 ]] && return
    sql "DELETE FROM activity_logs WHERE created_at < '$cutoff';" 2>/dev/null
    echo "[$(TS)] [CLEAN] $count ligne(s) supprimee(s) de activity_logs (avant $cutoff)" >> "$LOG_FILE"
}
