...
NGINXTEMP
      ...
      nft add rule inet kighmu input tcp dport 80 accept 2>/dev/null || true

      "${ACME_DIR}/acme.sh" --issue -d "${DOMAIN}" --webroot /var/www/html \
        --keylength ec-256 2>/tmp/acme.log

      # Supprimer la règle port 80
      local h80; h80=$(nft -a list chain inet kighmu input 2>/dev/null | grep 'dport 80' | grep -oP 'handle \K\d+' | tail -1)
      [[ -n "$h80" ]] && nft delete rule inet kighmu input handle "$h80" 2>/dev/null || true
...
NGINXHTTP
  ...
  if command -v ufw &>/dev/null; then
    ...
    if [[ $USE_TLS -eq 1 ]]; then
      TMP_NFT=$(mktemp)
      cat > "$TMP_NFT" << EOF
table inet kighmu-nginx {
    chain input {
        type filter hook input priority 0; policy accept;
        tcp dport { ${NGINX_PORT_TLS}, ${NGINX_PORT} } accept
    }
}
EOF
      if nft -c -f "$TMP_NFT"; then
        mv "$TMP_NFT" /etc/nftables/kighmu-nginx.nft
        systemctl daemon-reload
        systemctl enable --now nftables-tunnel@kighmu-nginx.service
        systemctl restart nftables-tunnel@kighmu-nginx.service
        ok "Firewall nftables : ports ${NGINX_PORT_TLS}(HTTPS) et ${NGINX_PORT}(redirect) ouverts"
      else
        rm -f "$TMP_NFT"
        ok "⚠️ Erreur nftables sur les ports nginx"
      fi
    else
      TMP_NFT=$(mktemp)
      cat > "$TMP_NFT" << EOF
...
EOF
      if nft -c -f "$TMP_NFT"; then
        ...
        systemctl enable --now nftables-tunnel@kighmu-nginx.service
        systemctl restart nftables-tunnel@kighmu-nginx.service
        ok "Firewall nftables : port ${NGINX_PORT} ouvert"
      else
        rm -f "$TMP_NFT"
        ok "⚠️ Erreur nftables sur le port nginx"
      fi
      ok "Firewall nftables : port ${NGINX_PORT} ouvert"
    fi
  fi
...
' ' ')
    ...
    # via nftables global (compteurs udp_zivpn_*/udp_hysteria_*) — pas dans cette boucle
    (( cycle++ ))
...
# ── SSH : delta via compteurs nftables ──────────────────────
# Les compteurs nommés ssh_<uid>_{out,in} sont créés par server.js (sshNftablesAdd)
# au reboot par Auto-clean.sh. Ici on lit uniquement le delta.
collect_ssh() {
  [ ! -f "\$USER_FILE" ] && return
  command -v nft &>/dev/null || return

  # Vérifier que la table kighmu existe
  if ! nft list table inet kighmu &>/dev/null; then
    echo "[\${TS}] [SSH] Table kighmu absente — en attente init par auto-clean.sh"
    return
  fi
  ...
  while IFS='|' read -r username _rest; do
    ...
    # Lire compteurs nftables
    local cur_out=0 cur_in=0
