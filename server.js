...
async function sshIptablesAdd(username) {
  try {
    ...
    console.log(`[SSH-RULES] regles iptables creees pour ${username} (uid=${uid})`);
  } catch (e) {
...
// ── Recrée les règles iptables SSH pour tous les clients actifs après reboot ──
async function syncAllSshRules() {
  try {
    ...
    let ok = 0;
    for (const r of rows) {
      try { await sshIptablesAdd(r.username); ok++; } catch {}
    }
    console.log(`[SSH-RULES] Sync apres reboot : ${ok}/${rows.length} regles iptables restaurees`);
  } catch (e) {
...
function startCron() {
  ...
  // ── Sync regles iptables SSH toutes les 10 min (recuperation apres reboot) ──
  setInterval(async () => {
